...
# Load both CSV files
file_a = "csv_update_deep_lite.csv"  # Replace with actual file paths
file_b = "csv_update_lite.csv"
...
def safe_eval(val):
    try:
        if isinstance(val, str) and val.startswith("[") and val.endswith("]"):
            return convert_np_objects(ast.literal_eval(val))  # Use ast.literal_eval
        return val
...
with open("predictions_lite.txt", "w", encoding="utf-8") as file:
