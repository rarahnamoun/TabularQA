...
def load_dataset(dataset_name, base_path="competition"):
    ...
    file_path = os.path.join(dataset_path, "sample.parquet")
...
def generate_prompt(question, dataset_name, df, previous_code=None, previous_error=None):
    ...
    sample_rows = df.head(21).to_dict(orient='records')
    ...
    prompt = f"""
        ...
        `competition/{dataset_name}/sample.parquet`
...
def update_csv(row_id, output, error_message, question, dataset_name):
    ...
    csv_update_path = "csv_update_deep_lite.csv"
...
if os.path.exists("csv_update_deep_lite.csv"):
    csv_update_df = pd.read_csv("csv_update_deep_lite.csv")
...
with open("predictions_deep-lite.txt", "a") as f:
